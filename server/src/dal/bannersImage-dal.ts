...
export const getBannersImage = async (): Promise<BannerInterface[]> => {
  ...
  export const getBannersImageByProductID = async (productID: string)=> {
      const data: BannerInterface[] | null = await bannerModel.findOne({ productID });
      if (!data) {throw new ApiError({}, STATUS_CODES.NOT_FOUND, "Banner by ID not found faild in .dal");}
      return data;
